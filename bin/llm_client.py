"""
Centralized LLM Client Component

This module provides a unified interface for all LLM API interactions
...
Currently supports OpenAI models, but designed to be extensible for other providers.
"""
...
class LLMClient:
    """
    Centralized LLM client for the Cybersecurity Agent.
    
    Provides a unified interface for all LLM API interactions with:
    ...
    Currently supports OpenAI models, but designed for extensibility.
    """
    ...
    def _initialize_client(self) -> bool:
        """Initialize the LLM client and test connectivity."""
        try:
            if not self.api_key:
                self.logger.warning("LLM API key not found - LLM features will be disabled")
                return False
    ...
    def generate_text(
        ...
        if not self.is_available():
            return LLMResponse(
                success=False,
                error="LLM client not available or not configured"
            )
...
def create_openai_llm_client(api_key: Optional[str] = None) -> LLMClient:
    """Backward compatibility alias for OpenAI LLM client."""
    return LLMClient(api_key)


def get_default_openai_llm_client() -> Optional[LLMClient]:
    """Backward compatibility alias for OpenAI LLM client."""
